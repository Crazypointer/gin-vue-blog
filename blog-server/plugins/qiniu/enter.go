...
// 获取上传的token
func getTocken(q config.QiNiu) string {
...
func UploadImage(data []byte, imageName string, prefix string) (filePath string, err error) {
	...
	if q.AccessKey == "" || q.SecretKey == "" {
		return "", errors.New("请配置accessKey或secretKey")
	}
	...
	err = formUploader.Put(context.Background(), &ret, upToken, key, bytes.NewBuffer(data), dataLen, &putExtra)
	if err != nil {
		return "", err
	}
